...
var localCurrency = d3.formatDefaultLocale({
  "decimal": ",",
  "thousands": ".",
  "grouping": [3],
  "currency": ["€", ""]
...
var localTime = d3.timeFormatLocale({
  "decimal": ",",
  "thousands": ".",
  "grouping": [3],
  "currency": ["€", ""],
...
var formatDates = localTime.format("%B %Y");
var currency = localCurrency.format("$,");
var trimColumns = function trimColumns(json, neededColumns) {

  // csv.columns = csv.columns.filter( (c) => {
  //     return neededColumns.indexOf(c) > -1;
  // });

  json.forEach(function (week, i) {
...
var ChartSVG = function ChartSVG(element, config, dimensions, svg) {
  ...
  var layers = function layers() {
    ...
    svg.layers.data = svg.body.append('g').attr('class', 'data').attr('transform', 'translate(' + config.padding.left + ',' + config.padding.top + ')');
    svg.layers.axes = svg.body.append('g').attr('class', 'axes');
    svg.layers.legend = svg.body.append('g').attr('class', 'legend');
  };
...
var ChartXScale = function ChartXScale(config, dimensions, scale) {

  var set = function set(data) {

    var endDate = new Date();

    scale.time = d3.scaleTime().domain([d3.min(data, function (d) {
      return new Date(d[config.xParameter]);
    }), //
    ...
    })).paddingInner(config.paddingInner).paddingOuter(config.paddingOuter).align([0.5]);

    return scale;
  };

  var reset = function reset(dimensions, newScale) {

    newScale.time.range([0, dimensions.width]);
...
var ChartYScale = function ChartYScale(config, dimensions, scale) {

  var set = function set(data, property) {
    ...
    if (config.minValue) {
      minValue = d3.max(data, function (d) {
        return d[property];
      }) > 20000 ? config.minValue : 900;
    } else {
      minValue = 0; //
    }

    scale.linear = d3.scaleLinear().domain([minValue, config.maxValue || d3.max(data, function (d) {
      return d[property];
    })]).nice();

    var arrayOfCumulatedValues = [];

    for (var p = 0; p < data.length; p++) {
      for (var i = 0; i < data[p].length - 1; i++) {
    ...
    scale.stacked = d3.scaleLinear().domain([minValue, config.maxValue || d3.max(arrayOfCumulatedValues)]).nice();
  ...
  var reset = function reset(dimensions, newScale) {
    ...
    newScale.linear.range([dimensions.height, 0]);

    newScale.stacked.range([config.fixedHeight || dimensions.height, 0]);
...
var ChartAxis = function ChartAxis(config, svg) {
  ...
  var redrawXBandAxis = function redrawXAxis(dimensions, xScale, axes, alternateTicks) {

    axes.xBand = d3.axisBottom(xScale.band);

    svg.xAxis.attr("transform", "translate(" + config.margin.left + "," + (dimensions.height + config.padding.top) + ")") //
    .call(axes.xBand);
    ...
    if (alternateTicks) {
      ...
      d3.selectAll("g.x-axis g.tick text").attr("y", function () {
        if (alternate_text) {
  ...
  var redrawXTimeAxis = function redrawXAxis(dimensions, scales, axes, ticks) {

    axes.xTime = d3.axisBottom(scales.time);

    if (ticks) {
      axes.xTime.ticks(d3.timeMonth.every(1)).tickFormat(function (date) {
        return d3.timeYear(date) < date ? localTime.format('%b')(date) : localTime.format('%Y')(date);
      });
    } else {
      axes.xTime.tickValues([]);
    }

    svg.xAxis.attr("transform", "translate(" + config.padding.left + "," + (dimensions.height + config.padding.top) + ")") //
    .call(axes.xTime);
  };

  var drawYAxis = function drawYAxis() {

    svg.yAxis = svg.layers.axes.append("g").attr('class', 'y-axis').attr("transform", "translate(" + parseInt(config.margin.left) + "," + (config.margin.top + config.padding.top) + ")");
  };

  var redrawYAxis = function redrawYAxis(yScale, axes) {

    axes.yLinear = d3.axisLeft(yScale.linear);

    axes.yLinear.ticks(5);

    svg.yAxis.call(axes.yLinear);
  };

  var redrawYAxisStacked = function redrawYAxisStacked(scales, axes) {
...
var ChartLegend = function ChartLegend(config, svg) {

  var drawInputLegend = function drawInputLegend(dimensions, data) {

    svg.legendTotals = svg.layers.legend.attr('transform', 'translate(' + (config.margin.left + config.padding.left) + ',' + config.margin.top + ')');

    svg.legendTotals.append("rect").attr('width', 8).attr('height', 8).attr('y', 0);

    svg.legendTotals.append("text").text('Een vierkantje staat voor 100 meldingen').attr('class', '').attr('height', 18).attr('x', 12).attr('y', 7);

    svg.legendTotals.selectAll("rect.type").data(data.reverse()).enter().append("rect").attr('class', function (d, i) {
      return 'type ' + sluggify(d[config.xParameter]);
    }).attr('width', 8).attr('height', 8).attr('y', function (d, i) {
      return i * 22 + 22;
    });

    svg.legendTotals.selectAll(".label").data(data).enter().append("text").attr('class', 'label').text(function (d) {
      return d[config.xParameter];
  ...
  var redrawInputLegend = function redrawInputLegend(dimensions, scales, axes) {};

  var drawDefault = function drawDefault(dimensions) {

    svg.legendDefault = svg.layers.legend.attr("transform", "translate(" + dimensions.height + ",30)").style("font-size", "12px").call(d3.legend);
  };

  var redrawDefault = function redrawDefault(dimensions) {};
...
var ChartLine = function ChartLine(config, svg, dimensions) {

  var popup = function popup(d) {

    return moment(d[config.xParameter]).subtract(1, 'week').format('D MMMM') + ' - ' + moment(d[config.xParameter]).format('D MMMM') + '<br/>' + d.increase + ' nieuwe meldingen' + '<br/>' + d.decrease + ' nieuwe besluiten';
  };
  ...
  var draw = function draw(data) {
    ...
    svg.candlesUp = svg.layers.data.selectAll('.candle up').data(data).enter().append("rect").attr("class", "candle up").style("fill", "orange");

    svg.candlesDown = svg.layers.data.selectAll('.candle down').data(data).enter().append("rect").attr("class", "candle down").style("fill", "#777c00");
  };

  var redraw = function redraw(xScale, yScale, functions, dimensions, data) {

    var candleWidth = dimensions.width / data.length - 4;

    functions.line = d3.line().x(function (d) {
      return xScale.time(new Date(d[config.xParameter]));
    }).y(function (d) {
      return yScale.linear(d[config.yParameter]);
    }).curve(d3.curveCardinal);

    svg.line.attr("d", functions.line);

    svg.candlesUp.attr('x', function (d) {
      return xScale.time(new Date(d[config.xParameter]));
    }).attr('y', function (d) {
      return yScale.linear(d[config.yParameter]) - (yScale.linear(0) - yScale.linear(d['nieuwe_schademeldingen']));
    }).attr('width', candleWidth).attr('height', function (d) {
      return yScale.linear(0) - yScale.linear(d['nieuwe_schademeldingen']);
    }).on("mouseover", function (d) {

      svg.tooltip.html(popup(d)).style("left", d3.event.pageX + 5 + "px").style("top", d3.event.pageY - 5 + "px").transition().duration(250).style("opacity", 1);
    }).on("mouseout", function (d) {
      svg.tooltip.transition().duration(250).style("opacity", 0);
    });

    svg.candlesDown.attr('x', function (d) {
      return xScale.time(new Date(d[config.xParameter]));
    }).attr('y', function (d) {
      return yScale.linear(d[config.yParameter]);
    }).attr('width', candleWidth).attr('height', function (d) {
      return yScale.linear(0) - yScale.linear(d['nieuwe_afgehandeld']);
    }).on("mouseover", function (d) {

      var html = moment(d[config.xParameter]).subtract(1, 'week').format('D MMMM') + ' - ' + moment(d[config.xParameter]).format('D MMMM') + '<br/>' + '' + d['nieuwe_schademeldingen'] + ' nieuwe meldingen' + '<br/>' + d['nieuwe_afgehandeld'] + ' nieuwe besluiten';
...
var ChartBar = function ChartBar(config, svg) {
  ...
  var draw = function draw(data, colours) {

    svg.bar = svg.layers.data.selectAll(".bar").data(data);

    svg.bar.exit().remove();

    svg.barEnter = svg.bar.enter().append("rect").attr("class", function (d, i) {
      return "bar  " + colours[i]; // + sluggify(d.status) + "
    });

    svg.barLabels = svg.layers.data.selectAll(".barLabel").data(data);

    svg.barLabels.exit().remove();

    svg.barLabelsEnter = svg.barLabels.enter().append('text').attr('class', 'barLabel small-label').attr('x', 0).attr('dx', '0px').attr('dy', '-6px').style("text-anchor", "middle");
  };

  var redraw = function redraw(dimensions, xScale, yScale) {

    svg.bar.merge(svg.barEnter).attr("x", function (d) {
      ...
      return config.margin.top + yScale.linear(d[config.yParameter]);
    }).attr("height", function (d) {
      return dimensions.height - yScale.linear(d[config.yParameter]);
    });

    svg.barLabels.merge(svg.barLabelsEnter).text(function (d) {
      ...
      if (config.xParameter === '_date') {
        ...
        return 'translate(' + xScale.time(new Date(d[config.xParameter])) + 60 + ',' + yScale.linear(d[config.yParameter]) + ')';
      } else {

        return 'translate(' + (xScale.band(d[config.xParameter]) + xScale.band.bandwidth() / 2) + ',' + yScale.linear(d[config.yParameter]) + ')';
      }
    }).attr('fill-opacity', 0).transition().delay(500).duration(500).attr('fill-opacity', 1);
  };
...
var ChartBarsIncrease = function ChartBarsIncrease(config, svg, functions) {
  ...
  var draw = function draw(data, colours) {
    ...
    svg.barEnter = svg.bar.enter().append("rect").attr("class", "bar");

    svg.barLabels = svg.layers.data.selectAll(".barLabel").data(data);

    svg.barLabels.exit().remove();

    svg.barLabelsEnter = svg.barLabels.enter().append('text').attr('class', 'barLabel small-label white').attr('x', 0).attr('dx', '0px').attr('dy', '-6px').style("text-anchor", "middle");

    svg.difference = svg.layers.data.selectAll(".diff").data(data);

    svg.difference.exit().remove();

    svg.differenceEnter = svg.difference.enter().append("rect").attr("class", "diff blue");

    svg.diffLabels = svg.layers.data.selectAll(".diffLabel").data(data);

    svg.diffLabels.exit().remove();

    svg.diffLabelsEnter = svg.diffLabels.enter().append('text').attr('class', 'diffLabel small-label blue').attr('x', 0).attr('dx', '0px').attr('dy', '-6px').style("text-anchor", "end");

    svg.dateLabels = svg.layers.data.selectAll(".dateLabel").data(data).enter().append('text').attr('class', 'dateLabel small-label').attr('x', 0).attr('dx', '0px').attr('dy', '22px').style("text-anchor", "middle");
  };

  var redraw = function redraw(dimensions, xScale, yScale, property) {

    var barWidth = 60; // scales.xBand.bandwidth() ||
    var yOffset = void 0;
    var xOffset = void 0;
    ...
    var minValue = d3.max(dataArray, function (d) {
      return d[property];
    }) > 20000 ? config.minValue : 900;

    svg.layers.data.append("defs").append("clipPath").attr("id", "clip").append("rect").attr("width", dimensions.width).attr("height", dimensions.height);
    ...
    svg.bar.merge(svg.barEnter).attr("x", function (d) {
      ...
      return dimensions.height;
    }).transition().duration(250).attr("y", function (d) {
      return yScale.linear(d[property]);
    }).attr("height", function (d) {
    ...
    svg.barLabels.merge(svg.barLabelsEnter).text(function (d) {
      ...
      return 'translate(' + (xScale.band(d[config.xParameter]) + barWidth / 2) + ',' + dimensions.height + ')';
      ...
      return 'translate(' + (xScale.band(d[config.xParameter]) + barWidth / 2) + ',' + (yScale.linear(d[property]) - yOffset) + ')';
    }).attr('fill-opacity', 1);

    svg.difference.merge(svg.differenceEnter).attr("y", function (d) {
      return yScale.linear(d[property]);
      ...
      return 10;
    }).attr("clip-path", "url(#clip)").transition().delay(250).duration(250).attr("height", function (d) {
    ...
    svg.diffLabels.merge(svg.diffLabelsEnter).text(function (d) {
      ...
      return 'translate(' + (xScale.band(d[config.xParameter]) - 20) + ',' + (yScale.linear(d[property]) + yOffset) + ')';
    }).attr('fill-opacity', 0).transition().delay(500).duration(500).attr('fill-opacity', 1);

    svg.dateLabels.merge(svg.dateLabels).text(function (d) {
      ...
      return 'translate(' + (xScale.band(d[config.xParameter]) + xOffset) + ',' + dimensions.height + ')';
    }).attr('fill-opacity', 0).transition().delay(500).duration(500).attr('fill-opacity', 1);
  };
...
var ChartStackedBars = function ChartStackedBars(config, svg, functions) {
  ...
  svg.defs = svg.layers.data.append("defs").append("clipPath").attr("id", "clip").append("rect");

  var draw = function draw(data, stackedData, colours) {

    dataArray = data;

    // series corresponds to provenance - the columns in the csv table//
    svg.series = svg.layers.data.selectAll(".stackGroup").data(stackedData);

    svg.series.exit().remove();

    svg.seriesEnter = svg.series.enter().append("g");

    svg.bar = svg.seriesEnter.merge(svg.series).selectAll("rect").data(function (d) {
      return d;
    });
    ...
    svg.barLabels = svg.seriesEnter.merge(svg.series).selectAll(".barLabel").data(function (d) {
      return d;
    });
    ...
    svg.dateLabels = svg.layers.axes.selectAll(".dateLabel").data(data);

    svg.dateLabelsEnter = svg.dateLabels.enter().append('text').attr('class', 'dateLabel small-label').attr('x', 0).attr('dx', config.padding.left).attr('dy', '52px').style("text-anchor", "middle").attr('fill-opacity', 0).transition().delay(500).duration(500).attr('fill-opacity', 1);
  };

  var redraw = function redraw(dimensions, xScale, yScale, colours) {
    ...
    svg.defs.attr("width", dimensions.width).attr("height", dimensions.height);

    svg.series = svg.seriesEnter.merge(svg.series).attr("class", function (d, i) {
      return "stackGroup " + colours[d.key];
    });

    svg.barEnter.attr("height", function (d) {
      return dimensions.height;
    });

    svg.barMerged = svg.barEnter.merge(svg.bar).attr("x", function (d) {
    ...
    }).attr("clip-path", "url(#clip)").transition().duration(500).attr("y", function (d) {
      return yScale.stacked(d[1]);
    }).attr("height", function (d) {
    ...
    svg.barLabels.merge(svg.barLabelsEnter).text(function (d) {
      ...
      xOffset = dimensions.width / (2 * dataArray.length);
      var start = d[0] < config.minValue ? config.minValue : d[0];
      yOffset = (yScale.stacked(d[1]) - yScale.stacked(start)) / 2 - 11;

      return 'translate(' + (xScale.band(d.data[config.xParameter]) + barWidth / 2) + ',' + (yScale.stacked(d[1]) - yOffset) + ')';
    }).attr('fill-opacity', 0).transition().delay(500).duration(500).attr('fill-opacity', 1);

    svg.dateLabels.merge(svg.dateLabelsEnter).text(function (d) {
...
var ChartStackedArea = function ChartStackedBars(config, svg, functions) {

  var draw = function draw(stackedData, colours) {
    ...
    svg.areas = svg.series.append("path")
    // .attr("fill", "#ccc")
    .attr('class', function (d, i) {
      return 'flow ' + colours[i];
    });

    svg.areaLabels = svg.series.append('text').datum(function (d) {
      return d;
    }).attr('x', 0).attr('dx', '-10px').attr('dy', '6px').style("text-anchor", "end").text(function (d) {
  ...
  var redraw = function redraw(dimensions, xScale, yScale) {
    ...
    var area = d3.area().x(function (d) {
      return xScale.time(new Date(d.data._date));
    }).y0(function (d) {
      return yScale.linear(d[0]);
    }).y1(function (d) {
      return yScale.linear(d[1]);
    });

    svg.areas.attr('d', area);

    svg.areaLabels.attr('transform', function (d) {

      return 'translate(' + dimensions.width + ',' + yScale.linear(d[d.length - 1][0] + .5 * (d[d.length - 1][1] - d[d.length - 1][0])) + ')';
    });
  };
...
var ChartBlocks = function ChartBlocks(config, svg, functions) {
  ...
  var draw = function draw(data) {
    ...
    var blocksArray = function blocksArray(d) {

      var noBlocks = Math.ceil(parseInt(d[config.yParameter]) / 100);

      if (Number.isInteger(noBlocks)) {

        var arr = new Array(noBlocks);
    ...
    var blockCount = function blockCount(value) {

      return Math.ceil(parseInt(value) / 100);
    };

    var blocksArrayTwo = function blocksArrayTwo(d, i) {

      var noBlocks = blockCount(d['value']);
      ...
      if (Number.isInteger(noBlocks)) {
        ...
        for (var j = 0; j < arr.length; j++) {
          ...
          if (j < blockCount(Object.keys(d).map(function (e) {
            return d[e];
          })[1])) {
            arr[j][config.xParameter] = Object.keys(d)[1];
            arr[j].class = sluggify(Object.keys(d)[1]);
          } else if (j < blockCount(Object.keys(d).map(function (e) {
            return d[e];
          })[1] + Object.keys(d).map(function (e) {
            return d[e];
          })[2])) {
            ...
            return d[e];
          })[2] + Object.keys(d).map(function (e) {
            return d[e];
          })[3])) {
    ...
    svg.blockGroup = svg.layers.data.selectAll("g.blocks").data(data).enter().append("g").attr("class", function (d, i) {
      return "blocks " + d[config.xParameter];
    });

    svg.blocks = svg.blockGroup.selectAll(".block").data(function (d, i) {
    ...
    }).enter().append("rect").attr("width", 8).attr("height", 8).attr("class", function (d, i) {
      return 'block ' + d.class;
    });
  };
  ...
  var redraw = function redraw(dimensions, scales) {
    ...
    svg.blockGroup.attr("transform", function (d, i) {
      return 'translate(' + 0 + ',' + (config.margin.top + dimensions.height) + ')';
    }).on("mouseout", unhighlight);

    svg.blocks.attr("x", function (d, i) {
      ...
      var s = Math.ceil(parseInt(i) + parseInt(d.previous) / 100).toString();
      ...
      if (s.length > 1) {
        var r = parseInt(s.substring(0, s.length - 1)) * 1000;
      ...
      var html = "<span class='uppercase'>" + d.qualifier + "</span><br/>" + d.total + " meldingen";

      svg.tooltip.html(html).style("left", d3.event.pageX + 5 + "px").style("top", d3.event.pageY - 5 + "px").transition().duration(250).style("opacity", 1);
    }).on("mouseout", function (d) {
      svg.tooltip.transition().duration(250).style("opacity", 0);
    });
  };
...
var ChartSankey = function ChartSankey(config, svg) {
  ...
  var set = function set(nodes, links, dimensions) {

    svg.sankey = d3.sankey().nodeWidth(24).nodePadding(40).size([dimensions.width, dimensions.height]);

    path = svg.sankey.link();

    svg.sankey.nodes(nodes).links(links).layout(64);
  };

  var draw = function draw(nodes, links) {

    // Set the sankey diagram properties


    svg.linkLayer = svg.layers.data.append("g").attr("class", "linkGroup");

    svg.links = svg.linkLayer.selectAll(".link").data(links);

    svg.links.exit().remove();

    svg.linksEnter = svg.links.enter().append("path").attr("class", function (d) {
      return 'link ' + d.class;
    });

    svg.nodeLayer = svg.layers.data.append("g").attr("class", "nodeGroup");

    svg.nodeGroup = svg.nodeLayer.selectAll('.node').data(nodes);

    svg.nodeGroup.exit().remove();

    svg.nodeGroupEnter = svg.nodeGroup.enter().append("g").attr("class", "node");

    svg.nodeRect = svg.nodeGroupEnter.append("rect").style("fill", function (d) {
    ...
    svg.nodeName = svg.nodeGroupEnter.append("text").attr("x", -6).attr("dy", ".5em").attr("transform", null).text(function (d) {
      return translate(d.name);
    }).attr("text-anchor", "start");

    svg.nodeValue = svg.nodeGroupEnter.append("text").attr("x", -6).attr("dy", "-.5em").attr("transform", null);
  };
  ...
  var redraw = function redraw(dimensions) {
    ...
    // add in the links
    svg.links.merge(svg.linksEnter).attr("d", path)
    ...
    // add in the nodes
    svg.nodeGroup.merge(svg.nodeGroupEnter).attr("transform", function (d) {
      return "translate(" + d.x + "," + d.y + ")";
    });

    svg.nodeRect.style("opacity", 0.4).attr("height", function (d) {
      return d.dy;
    }).attr("width", svg.sankey.nodeWidth()).transition().delay(500).duration(1500).style("opacity", 1);
    ...
    svg.nodeName.attr("x", function (d) {
    ...
    svg.nodeValue.attr("y", function (d) {
      return d.dy / 2;
    }).attr("x", function (d) {
      return d.x < dimensions.width / 2 ? 6 + svg.sankey.nodeWidth() : -6;
    }).attr("text-anchor", function (d) {
...
var ChartMap = function ChartMap(config, svg, dimensions) {
  ...
  var draw = function draw(features) {

    svg.map = svg.layers.data.selectAll("path").data(features).enter().append("path").attr("class", function (d, i) {
      return sluggify(d.properties.gemeentenaam);
    }).attr("d", path).attr("stroke", "#fff");

    svg.values = svg.layers.data.selectAll(".value").data(features).enter().append("text").attr("class", "value small-label").attr("x", function (d) {
  ...
  var redraw = function redraw(dimensions, property) {

    svg.map.merge(svg.map).attr("fill", function (d, i) {
      ...
      if (d.properties[property]) {
        var ratio = .8 * d.properties[property] / 1500;
        return ratio + 0.2;
      } else {
        return 1;
      }
      ...
      var html = "<div class='uppercase'>" + d.properties.gemeentenaam + "</div><div>" + d.properties[property] + "</div>";
...
var opnames = function opnames(element) {

  var options = [].slice.call(document.querySelectorAll('.selector li input[type=checkbox]'));
  var radios = [].slice.call(document.querySelectorAll('.selector li input[type=radio]'));
  ...
  var xScale = chartObjects.xScale();
  var yScale = chartObjects.yScale();
  ...
  var chartXScale = ChartXScale(config, dimensions, xScale);
  var chartYScale = ChartYScale(config, dimensions, yScale);
  var chartAxis = ChartAxis(config, svg);
  var chartStackedBars = ChartStackedBars(config, svg);
  var chartLegend = ChartLegend(config, svg);

  chartAxis.drawXAxis();
  chartAxis.drawYAxis();

  var url = 'https://tcmg.publikaan.nl/api/procedure';
  ...
  d3.json(url, function (error, json) {
    ...
    var propertyArray = ['schadeopnames'];
    ...
    function filterData(array, increments) {
      ...
      if (increments) {
        ...
        for (var _i = 0; _i < array.length; _i++) {
          array[_i] = array[_i].slice(0, 7) === 'nieuwe_' ? array[_i].slice(7, array[_i].length) : array[_i];
        }
      }

      functions.stack = d3.stack().keys(Object.keys(data[data.length - 1]).filter(function (key) {
    ...
    function update(propertyArray, increments) {
      ...
      var stackedData = filterData(propertyArray, increments);
      ...
      var _loop = function _loop() {
        ...
        option.addEventListener('click', function () {
          if (option.checked) {
            ...
            var index = propertyArray.indexOf(option.value);
    ...
    try {
      for (var _iterator2 = radios[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
...
var meldingen = function meldingen(element) {
  ...
  var chartYScale = ChartYScale(config, dimensions, yScale);
  var chartAxis = ChartAxis(config, svg);
  var chartBarsIncrease = ChartBarsIncrease(config, svg);
  var chartLegend = ChartLegend(config, svg);
  ...
  d3.json(url, function (error, json) {
    ...
    try {
      ...
      for (var _iterator3 = radios[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        _loop2();
      }
    } catch (err) {
...
var weekverloop = function weekverloop(element) {
  ...
  var chartAxis = ChartAxis(config, svg);
  var chartStackedArea = ChartStackedArea(config, svg, functions);
  ...
  d3.json(url, function (error, json) {
    ...
    var neededColumns = ['DATUM', 'MELDING', 'BESCHIKT', '_date'];

    var data = trimColumns(json, neededColumns);

    functions.stack = d3.stack()
    // do not stack DATUM
    .keys(Object.keys(data[0]).slice(1, 3));

    var stackedData = functions.stack(data);
...
var gemeentes = function gemeentes(element) {
  ...
  var chartMap = ChartMap(config, svg, dimensions);

  var radios = [].slice.call(document.querySelectorAll('.map-selector ul li input[type=radio]'));

  d3.json("/assets/geojson/topojson.json", function (error, mapData) {

    var features = topojson.feature(mapData, mapData.objects.gemeenten).features;

    var url = 'https://tcmg.publikaan.nl/api/gemeentes';
    ...
    d3.json(url, function (error, json) {
      ...
      try {
        ...
        for (var _iterator4 = radios[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
          _loop3();
        }
      } catch (err) {
...
var stroommodel = function stroommodel(element) {

  var options = [].slice.call(document.querySelectorAll('.selector li input[type=checkbox]'));
  var radios = [].slice.call(document.querySelectorAll('.selector li input[type=radio]'));

  var colours = ['green', 'orange', 'blue'];
  var url = "https://tcmg.publikaan.nl/api/flowchart?week=recent";
  ...
  var chartSankey = ChartSankey(config, svg, functions);

  var connections = [["VOOR_19MRT", "VOOR_19MRT_MET_HISTORIE", "HISTORIE", ""], ["VOOR_19MRT", "VOOR_19MRT_GEEN_HISTORIE", "MELDING", ""], ["VOOR_19MRT", "AOS_VR_19MRT_MET_HISTORIE", "aos_totaal", ""], ["19MRT_30SEPT", "19MRT_TM_30SEPT_GEEN_HIST", "MELDING", ""], ["19MRT_30SEPT", "19MRT_TM_30SEPT_MET_HISTO", "HISTORIE", ""], ["19MRT_30SEPT", "AOS_REGULIER_19MRT_30SEPT", "aos_totaal", ""], ["VANAF_O1OKT2018", "VANAF_O1OKT2018_GEEN_HIST", "MELDING", ""], ["VANAF_O1OKT2018", "VANAF_010KT2018_MET_HISTO", "HISTORIE", ""], ["VANAF_O1OKT2018", "AOS_REGULIER_VANAF_1OKT20", "aos_totaal", ""],

  //  ["GEEN_HISTORIE", "GEEN_HISTORIE", "MELDING"],
  ["HISTORIE", "HISTORIE", "MELDING", ""], ["MELDING", "OPNAMES", "OPNAMES", ""], ["MELDING", "AANTAL_BESLUIT_STUWMEER", "STUWMEERREGELING", "completed"], ["MELDING", "GEEN_REACTIE_STUWMEER", "STUWMEERREGELING", "uncompleted"], ["MELDING", "WACHT_OP_OPNAME", "IN_PROCEDURE", "in-procedure"], ["STUWMEERREGELING", "GEEN_REACTIE_STUWMEER", "IN_PROCEDURE", "in-procedure"], // 0%
  ["STUWMEERREGELING", "AANTAL_BESLUIT_STUWMEER", "AANTAL_BESLUIT_STUWMEER", ""], // 100%


  ["aos_totaal", "", "", ""], ["OPNAMES", "RAPPORTAGES", "RAPPORTAGES", ""], ["OPNAMES", "WACHT_OP_RAPPORT", "IN_PROCEDURE", "in-procedure"], ["RAPPORTAGES", "BESLUITEN", "BESLUITEN", ""], ["RAPPORTAGES", "WACHT_OP_BESLUIT", "IN_PROCEDURE", "in-procedure"], ["BESLUITEN", "AANTAL_AFGEWEZEN", "AANTAL_AFGEWEZEN", ""], ["BESLUITEN", "AANTAL_GEDEELTELIK_TOEGEK", "AANTAL_GEDEELTELIK_TOEGEK", ""], ["BESLUITEN", "AANTAL_GEENCONSISTENT_BES", "AANTAL_GEENCONSISTENT_BES", ""], ["BESLUITEN", "AANTAL_GEHEEL_TOEGEKEND", "AANTAL_GEHEEL_TOEGEKEND", ""], ["AANTAL_AFGEWEZEN", "", "", ""], ["AANTAL_GEDEELTELIK_TOEGEK", "", "", ""], ["AANTAL_GEENCONSISTENT_BES", "", "", ""], ["AANTAL_GEHEEL_TOEGEKEND", "", "", ""], ["AANTAL_BESLUIT_STUWMEER", "", "", ""], ["IN_PROCEDURE", "", "", ""]];
...
var bedragen = function bedragen(element) {

  var totalElement = document.querySelector('h2 span');

  var colours = ['green', 'green', 'green', 'green', 'green'];
  ...
  var chartBar = ChartBar(config, svg, functions);

  var url = 'https://tcmg.publikaan.nl/api/schadevergoedingen?week=recent';
  ...
  d3.json(url, function (error, json) {
    ...
    function run(json, filter) {
      var data = prepareData(json, filter);
      draw(data);
      redraw();
      totalElement.innerText = convertToCurrency(json.filter(function (j) {
        return j['_category'] === filter;
      })[0]['TOTAAL_VERLEEND']);
    }
...
var verdeling = function verdeling(element) {

  var colours = ['green', 'green', 'green', 'green'];
...
var voortgang = function voortgang(element) {
  ...
  d3.json(url, function (error, json) {
    ...
    var neededColumns = ['DATUM', 'afgehandeld', 'in_behandeling', '_date'];
...
var candles = function candles(element) {
  ...
  var chartYScale = ChartYScale(config, dimensions, yScale);
  var chartAxis = ChartAxis(config, svg);
  var chartLine = ChartLine(config, svg, functions, dimensions);
  ...
  d3.json(url, function (error, json) {
    ...
    var neededColumns = ['schademeldingen', 'afgehandeld', 'in_behandeling', 'nieuwe_schademeldingen', 'nieuwe_afgehandeld', '_date'];
    var data = trimColumns(json, neededColumns);
...
var statussen = function statussen(element, filter) {

  var municipalitySelect = document.querySelector('select.municipalities');
  ...
  var colours = ['orange', 'green', 'darkblue', 'blue', 'orange', 'green'];
  ...
  var chartYScale = ChartYScale(config, dimensions, yScale);
  var chartAxis = ChartAxis(config, svg);
  var chartBar = ChartBar(config, svg);
...
var statussen_stroommodel = function statussen_stroommodel(element, filter) {
  ...
  var connections = [['ONTVANGST', 'PLANNING_OPNAME', 'PLANNING_OPNAME']];
...
var statussen_stroommodel_ii = function statussen_stroommodel_ii(element, filter) {
