let ChartStackedBars = function ChartStackedBars(config,svg,functions) {
    ...
    let redraw = function redraw(dimensions,scales) {
        ...
        let area = d3.area()
            ...
            .x0((d,i) => { if (i < 1) { console.log(scales.xBand(d[0].data.status)); return  scales.xBand(d[0].data.status) + barWidth } else { return scales.xBand(d[0].data.status);}})  // console.log(d);
            .x1((d,i) => { if (i < 1) { console.log(scales.xBand(d[1].data.status)); return scales.xBand(d[1].data.status) + barWidth } else { return scales.xBand(d[1].data.status); }})
