let ChartScales = function ChartScales(config,dimensions,scales) {
    ...
    let set = function set(data) {
        ...
        scales.xTime = d3.scaleTime()
            .domain([
                d3.min(data, d => { console.log(new Date(d[config.xParameter])); new Date(d[config.xParameter]) }),
        ...
        scales.yLinear = d3.scaleLinear()
            ...
            .range([(config.fixedHeight || dimensions.height), 0]) // geen idee waarom 259 ipv 250
            .domain([
                config.minValue,
