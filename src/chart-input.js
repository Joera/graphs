let ChartInput = function ChartInput(config,svg,functions) {
    ...
    let redraw = function redraw(dimensions,scales) {
        ...
        svg.inputRects
            .attr("y", (d,i) => { let s = (parseInt(i) + (parseInt(d.previous) / 100)).toString(); console.log(s); if (s.length > 1) { return dimensions.height - 6 - (10 * parseInt(s.substring(0,s.length - 1))); } else { return dimensions.height - 6; }});
